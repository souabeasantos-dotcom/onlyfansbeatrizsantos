exports.handler = async (event) => {
  ...
  try {
    ...
    if (text === '/start' || text === '/START') {
      const keyboard = { inline_keyboard: [[{ text: '🔥 LIBERAR MEU ACESSO - R$10 🔥', callback_data: 'pagar' }]] };
      await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ chat_id: chatId, text: '👋 Bem-vinda ao VIP da Bea Santos!\n\nClique abaixo para liberar seu acesso completo por apenas R$10:', reply_markup: keyboard })
      });
    }
    ...
    if (text === 'pagar') {
      ...
      const pixRes = await fetch('https://api.pushinpay.com.br/api/pix/cashIn', {
        ...
        body: JSON.stringify({ value: 1000, webhook_url: `${siteUrl}/.netlify/functions/pushinpay-webhook` })
      });
      const pixData = await pixRes.json();
      await supabase.from('pagamentos').insert({ uid: uid, telegram_id: String(chatId), pix_id: pixData.id, status: 'pending', valor: 10 });
      await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ chat_id: chatId, text: `💳 PIX gerado!\n\nCopia e cola no seu banco:\n\n\`${pixData.qr_code || pixData.qr_code_text || pixData.pix_qrcode}\`\n\nApós pagar, digite /verificar`, parse_mode: 'Markdown' })
      });
    }
    ...
    if (text === '/verificar') {
      ...
      if (pago || pagamento.status === 'paid') {
        ...
        const link = `${siteUrl}/vip.html?uid=${pagamento.uid}`;
        await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ chat_id: chatId, text: `✅ Pagamento confirmado!\n\nSeu acesso VIP:\n${link}` })
        });
      } else {
        await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
